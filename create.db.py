...
with app.app_context():
    ...
    users = [
        ...
        {'username': 'thyabe77', 'email': 'thyabe77@terpmail.umd.edu', 'first_name':'Abraam', 'last_name':'Benitez',
            'password': generate_password_hash('thyabe77', method='pbkdf2:sha256'), 'role':'STUDENT'}
    ]


    for each_user in users:
        print(f'{each_user["username"]} inserted into user')
        a_user = User(username=each_user["username"], email=each_user["email"], first_name=each_user["first_name"], last_name=each_user["last_name"], password=each_user["password"], role=each_user["role"])
